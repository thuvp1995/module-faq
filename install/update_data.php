...
// ID goi cap nhat
$nv_update_config['packageID'] = 'NVUDFAQ4029';
...
$nv_update_config['support_website'] = 'https://github.com/nukeviet/module-faq/tree/4.0.29';
...
function nv_up_p1()
{
    ...
    foreach ($array_modlang_update as $lang => $array_mod) {
        foreach ($array_mod['mod'] as $module_info) {
            ...
            try {
                $db->query("ALTER TABLE " . $table_prefix . " DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
            } catch (PDOException $e) {
                trigger_error($e->getMessage());
            }
            try {
                $db->query("ALTER TABLE " . $table_prefix . "_categories DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
            } catch (PDOException $e) {
                trigger_error($e->getMessage());
            }
            try {
                $db->query("ALTER TABLE " . $table_prefix . "_config DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
            } catch (PDOException $e) {
                trigger_error($e->getMessage());
            }
            ...
            // Sửa lại các bảng
            try {
                $db->query("ALTER TABLE " . $table_prefix . " CHANGE title title VARCHAR(250) NOT NULL DEFAULT ''");
                $db->query("ALTER TABLE " . $table_prefix . " CHANGE alias alias VARCHAR(250) NOT NULL DEFAULT ''");
            } catch (PDOException $e) {
                trigger_error($e->getMessage());
            }
            try {
                $db->query("ALTER TABLE " . $table_prefix . "_categories CHANGE title title VARCHAR(250) NOT NULL DEFAULT ''");
                $db->query("ALTER TABLE " . $table_prefix . "_categories CHANGE alias alias VARCHAR(250) NOT NULL DEFAULT ''");
            } catch (PDOException $e) {
                trigger_error($e->getMessage());
            }            

            // Cập nhật chasert từng cột của các bảng
            $array_table = array('', '_categories', '_config');
    ...
    try {
        ...
        if (!$num) {
            $db->query("INSERT INTO " . $db_config['prefix'] . "_setup_extensions (
                ...
                28, 'module', 'faq', 0, 1, 'faq', 'faq', '4.0.29 1463590800', " . NV_CURRENTTIME . ", 'VINADES.,JSC (contact@vinades.vn)', 
                'Hỗ trợ hỏi đáp'
            )");
        } else {
